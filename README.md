...
- a platform supported by `tools/fetch_croc.py`: Linux x86_64/ARM64, macOS
  Intel/Apple Silicon, or Windows 64-bit.
...
prerequisites, run `uv sync`, and then call `tools/build.py`.
...
- make the build more reproducible by optionally pinning the `croc` version.
...
`tools/fetch_croc.py` downloads the latest `croc` release, verifies the archive
checksum, and copies the binary into `third_party/croc/`.
...
send/receive arguments, and handling of the `CROC_SECRET` variable.
